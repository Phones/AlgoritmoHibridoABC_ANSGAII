...
class cromossomo:
    ...
    def pega_modulos(self):
        self.lista_modulos = [0 for i in range(len(G.arestas))]
        tam = len(G.arestas)
        ...
        for e in range(tam):
            ...
            for k in range(len(G.arestas[e].module_list)):
                if G.arestas[e].module_list[k].capacidade + G.arestas[e].pre_installed_capacity >= self.fluxo_aresta[e]:
                    if G.arestas[e].module_list[k].cost < val:
                        val = G.arestas[e].module_list[k].cost
                        self.lista_modulos[e] = G.arestas[e].module_list[k].capacidade
    ...
    def __calcula_custo(self):
        ...
        for e in range(len(G.arestas)):
            ...
            if self.fluxo_aresta == 0 or self.fluxo_aresta[e]<=G.arestas[e].pre_installed_capacity:
                self.custo += self.fluxo_aresta[e]*G.arestas[e].routing_cost
                continue
            val = 100000000000000000.0
            for k in range(len(G.arestas[e].module_list)):
                if G.arestas[e].module_list[k].capacidade+G.arestas[e].pre_installed_capacity >= self.fluxo_aresta[e]:
    ...
    def __calcula_custo_dif(self,fluxo_aresta):
        ...
        for e in range(len(G.arestas)):
            ...
            if fluxo_aresta == 0 or fluxo_aresta[e]<=G.arestas[e].pre_installed_capacity:
                custo += fluxo_aresta[e]*G.arestas[e].routing_cost
                continue
        ...
        while continua:
            ...
            while demanda_idx < len(self.demanda_caminho):
                ...
                for cmh in conjunto_caminho:  # para cada caminho do conjunto de caminhos
                    ...
                    if (custo < self.custo and soma_tam_caminho <= self.soma_tam_caminhos) or (
                            custo <= self.custo and soma_tam_caminho < self.soma_tam_caminhos):
...
class GA:
    def __init__(self, populacaoABC=None, quant_indv=None):
        arq = open("salva_frentes.txt", "w")
        arq.write("a\n")
        arq.close()

        self.quant_indv = quant_indv
        self.populacaoABC = populacaoABC
        self.populacao = []
        self.tamanho_populacao = 75 + self.quant_indv
        print("Tamanho da população: ", self.tamanho_populacao)
        self.menor_custo = [1000000000000000, cromossomo()] 
    ...
    def gera_imagem_soluc(self):
        obj_visual = visual()
        for i in range(len(grande_frente_pareto)):
            obj_visual.plota_grafico(G,grande_frente_pareto[i].fluxo_aresta,i)
            grande_frente_pareto[i].pega_modulos()
            print('------Solucao',i + 1,'--------')
            for j in range(len(G.arestas)):
                print('fluxo: ',grande_frente_pareto[i].fluxo_aresta[j],'capacidade instalada',grande_frente_pareto[i].lista_modulos[j])
            print('----------------------------')

    def insere_indv_ABC(self):
        for i in range(self.quant_indv):
            self.populacao[i] = self.populacaoABC[i]

    # Mescla a população do GA com o ABC
    def mescla_populacoes(self):
        for i in range(self.quant_indv):
            self.populacao.append(cromossomo(tipo="atribuir", item=self.populacaoABC[i]))

        self.tamanho_populacao = len(self.populacao)

    def troca_indv_ga_por_abc(self):
        for i in range(self.quant_indv):
            self.populacao[i].demanda_caminho = self.populacaoABC[i].demanda_caminho
            self.populacao[i].fluxo_aresta = self.populacaoABC[i].fluxo_aresta
            self.populacao[i].custo = self.populacaoABC[i].custo
            self.populacao[i].soma_tam_caminhos = self.populacaoABC[i].soma_tam_caminhos

        self.tamanho_populacao = len(self.populacao)

    def get_menor_custo(self):
        for p in self.populacao:
            if p.custo < self.menor_custo[0]:
                self.menor_custo = [p.custo, p]


    def salva_arquivo(self, stringg____):
        arq = open("salva_frentes.txt", 'r')
        texto = arq.read()
        arq.close()

        arq = open("salva_frentes.txt", 'w')
        texto += str(stringg____) + "\n"
        arq.write(texto)
        arq.close()



    def Executa(self, total_tempo_execucaoAG):
        tempo_inicial = time.process_time()
        self.__gera_populacao()
        print("Tamhno da população gerada: ", len(self.populacao))
        #self.troca_indv_ga_por_abc()
        self.mescla_populacoes()
        print("Tamanho da população depois de inserir os individuos do ABC: ", len(self.populacao))
        self.frente = []
        geracao = 0
